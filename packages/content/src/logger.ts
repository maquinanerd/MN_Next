 ...
 *
 * It lives in the content package rather than in the app because the events worth an
 * alert happen here: `KalElTransport.fromEnv()` wires it in by default, so a contract
 * violation reaches the log whoever constructs the transport. It used to be the app's,
 * and the transport's `onLog` defaulted to a no-op that nothing ever replaced — the
 * RUNBOOK's one "wake someone up" signal was never written. `lib/logger.ts` re-exports it.
 */
